...
public class EntityHorseSaddleFix extends NamedEntityFix {
   ...
   protected Typed<?> fix(Typed<?> pTyped) {
      ...
      Optional<? extends Typed<?>> optional = pTyped.getOptionalTyped(opticfinder1);
      Dynamic<?> dynamic = pTyped.get(DSL.remainderFinder());
      if (!optional.isPresent() && dynamic.get("Saddle").asBoolean(false)) {
         Typed<?> typed = type.pointTyped(pTyped.getOps()).orElseThrow(IllegalStateException::new);
         ...
         return pTyped.set(opticfinder1, typed).set(DSL.remainderFinder(), dynamic);
      } else {
         return pTyped;
      }
   }
