...
public class ChunkProtoTickListFix extends DataFix {
   ...
   private Dynamic<?> makeTickList(Dynamic<?> pDynamic, Int2ObjectMap<Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer>> p_185038_, byte p_185039_, int pX, int pZ, String pName, Function<Dynamic<?>, String> p_185043_) {
      Stream<Dynamic<?>> stream = Stream.empty();
      List<? extends Dynamic<?>> list = pDynamic.get(pName).asList(Function.identity());
      ...
      for(int i = 0; i < list.size(); ++i) {
         ...
         Stream<? extends Dynamic<?>> stream1 = list.get(i).asStream().mapToInt((p_185074_) -> {
            ...
            return this.createTick(pDynamic, supplier, pX, j, pZ, p_185059_, p_185043_);
         });
         stream = Stream.concat(stream, stream1);
      }

      return pDynamic.createList(stream);
   }
   ...
   private Dynamic<?> createTick(Dynamic<?> pDynamic, @Nullable Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer> p_185046_, int pX, int pY, int pZ, int p_185050_, Function<Dynamic<?>, String> p_185051_) {
      int i = p_185050_ & 15;
      int j = p_185050_ >>> 4 & 15;
      int k = p_185050_ >>> 8 & 15;
      String s = p_185051_.apply(p_185046_ != null ? p_185046_.get().get(i, j, k) : null);
      return pDynamic.createMap(ImmutableMap.<Dynamic<?>, Dynamic<?>>builder().put(pDynamic.createString("i"), pDynamic.createString(s)).put(pDynamic.createString("x"), pDynamic.createInt(pX * 16 + i)).put(pDynamic.createString("y"), pDynamic.createInt(pY * 16 + j)).put(pDynamic.createString("z"), pDynamic.createInt(pZ * 16 + k)).put(pDynamic.createString("t"), pDynamic.createInt(0)).put(pDynamic.createString("p"), pDynamic.createInt(0)).build());
   }
