...
public class VillagerTradeFix extends NamedEntityFix {
   public VillagerTradeFix(Schema pOutputSchema, boolean pChangesType) {
      super(pOutputSchema, pChangesType, "Villager trade fix", References.ENTITY, "minecraft:villager");
   }

   protected Typed<?> fix(Typed<?> pTyped) {
      OpticFinder<?> opticfinder = pTyped.getType().findField("Offers");
      ...
      if (!(type instanceof List.ListType<?> listtype)) {
         ...
         return pTyped.updateTyped(opticfinder, (p_17125_) -> {
   ...
   private Typed<?> updateItemStack(OpticFinder<Pair<String, String>> p_17134_, Typed<?> pTyped) {
      return pTyped.update(p_17134_, (p_17145_) -> {
