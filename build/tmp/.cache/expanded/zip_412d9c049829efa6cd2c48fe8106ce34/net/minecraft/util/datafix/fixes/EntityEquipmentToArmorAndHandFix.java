...
public class EntityEquipmentToArmorAndHandFix extends DataFix {
   ...
   private <IS> TypeRewriteRule cap(Type<IS> pItemStackType) {
      ...
      Type<Pair<Either<List<IS>, Unit>, Pair<Either<List<IS>, Unit>, Dynamic<?>>>> type1 = DSL.and(DSL.optional(DSL.field("ArmorItems", DSL.list(pItemStackType))), DSL.optional(DSL.field("HandItems", DSL.list(pItemStackType))), DSL.remainderType());
      ...
      return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(References.ENTITY), this.getOutputSchema().getType(References.ENTITY), (p_15425_) -> {
         ...
         if (optional.isPresent()) {
            ...
            IS is = pItemStackType.read(dynamic.emptyMap()).result().orElseThrow(() -> {
