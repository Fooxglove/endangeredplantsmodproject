...
public class FurnaceRecipeFix extends DataFix {
   ...
   private <R> TypeRewriteRule cap(Type<R> pType) {
      Type<Pair<Either<Pair<List<Pair<R, Integer>>, Dynamic<?>>, Unit>, Dynamic<?>>> type = DSL.and(DSL.optional(DSL.field("RecipesUsed", DSL.and(DSL.compoundList(pType, DSL.intType()), DSL.remainderType()))), DSL.remainderType());
      ...
      return this.fixTypeEverywhereTyped("FurnaceRecipesFix", type4, type5, (p_15848_) -> {
         return p_15848_.updateTyped(opticfinder, type1, (p_145372_) -> {
            return this.updateFurnaceContents(pType, type, p_145372_);
         }).updateTyped(opticfinder1, type2, (p_145368_) -> {
            return this.updateFurnaceContents(pType, type, p_145368_);
         }).updateTyped(opticfinder2, type3, (p_145364_) -> {
            return this.updateFurnaceContents(pType, type, p_145364_);
         });
      });
   }

   private <R> Typed<?> updateFurnaceContents(Type<R> pType, Type<Pair<Either<Pair<List<Pair<R, Integer>>, Dynamic<?>>, Unit>, Dynamic<?>>> p_15853_, Typed<?> p_15854_) {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         if (k > 0) {
            optional.ifPresent((p_15859_) -> {
               Optional<? extends Pair<R, ? extends Dynamic<?>>> optional1 = pType.read(p_15859_).result();
