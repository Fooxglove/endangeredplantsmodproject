...
public class V99 extends Schema {
   ...
   public void registerTypes(Schema pSchema, Map<String, Supplier<TypeTemplate>> p_18308_, Map<String, Supplier<TypeTemplate>> p_18309_) {
      ...
      pSchema.registerType(true, References.ITEM_STACK, () -> {
         return DSL.hook(DSL.optionalFields("id", DSL.or(DSL.constType(DSL.intType()), References.ITEM_NAME.in(pSchema)), "tag", DSL.optionalFields("EntityTag", References.ENTITY_TREE.in(pSchema), "BlockEntityTag", References.BLOCK_ENTITY.in(pSchema), "CanDestroy", DSL.list(References.BLOCK_NAME.in(pSchema)), "CanPlaceOn", DSL.list(References.BLOCK_NAME.in(pSchema)), "Items", DSL.list(References.ITEM_STACK.in(pSchema)))), ADD_NAMES, HookFunction.IDENTITY);
      });
   ...
   protected static <T> T addNames(Dynamic<T> pDynamic, Map<String, String> pRenamer, String pId) {
      return pDynamic.update("tag", (p_145917_) -> {
         return p_145917_.update("BlockEntityTag", (p_145912_) -> {
            String s = pDynamic.get("id").asString().result().map(NamespacedSchema::ensureNamespaced).orElse("minecraft:air");
            if (!"minecraft:air".equals(s)) {
               String s1 = pRenamer.get(s);
               if (s1 != null) {
                  return p_145912_.set("id", pDynamic.createString(s1));
               }
            ...
            String s = pDynamic.get("id").asString("");
            return "minecraft:armor_stand".equals(NamespacedSchema.ensureNamespaced(s)) ? p_145908_.set("id", pDynamic.createString(pId)) : p_145908_;
         });
      }).getValue();
   }
}