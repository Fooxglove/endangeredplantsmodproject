...
public class ChunkHeightAndBiomeFix extends DataFix {
   ...
   private Dynamic<?> predictChunkStatusBeforeSurface(Dynamic<?> pDynamic, Set<String> pBlockPalette) {
      return pDynamic.update("Status", (p_184919_) -> {
   ...
   private static Dynamic<?>[] getBiomeContainers(Dynamic<?> pDynamic, boolean pOverworld, int p_184909_, MutableBoolean p_184910_) {
      Dynamic<?>[] dynamic = new Dynamic[pOverworld ? 24 : 16];
      int[] aint = pDynamic.get("Biomes").asIntStreamOpt().result().map(IntStream::toArray).orElse((int[])null);
      if (aint != null && aint.length == 1536) {
         p_184910_.setValue(true);

         for(int l = 0; l < 24; ++l) {
            int l_f = l;
            dynamic[l] = makeBiomeContainer(pDynamic, (p_184967_) -> {
         ...
         for(int i = 0; i < 16; ++i) {
            int i_f = i;
            int j = i - p_184909_;
            dynamic[j] = makeBiomeContainer(pDynamic, (p_184954_) -> {
               return getOldBiome(aint, i_f * 64 + p_184954_);
            });
         }

         if (pOverworld) {
            Dynamic<?> dynamic1 = makeBiomeContainer(pDynamic, (p_184976_) -> {
               return getOldBiome(aint, p_184976_ % 16);
            });
            Dynamic<?> dynamic2 = makeBiomeContainer(pDynamic, (p_184963_) -> {
         ...
         Arrays.fill(dynamic, makePalettedContainer(pDynamic.createList(Stream.of(pDynamic.createString("minecraft:plains")))));
      }
   ...
   private static int getOldBiome(int[] pBiomes, int pIndex) {
      return pBiomes[pIndex] & 255;
   }

   private static Dynamic<?> updateChunkTag(Dynamic<?> pChunkTag, boolean pOverworld, boolean p_184914_, boolean pIsNoiseGenerator, Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer> p_184916_) {
      ...
      if (!pOverworld) {
         ...
         if (!pIsNoiseGenerator) {
            ...
            if (optional.isPresent()) {
               ...
               if (!"empty".equals(s)) {
                  pChunkTag = pChunkTag.set("blending_data", pChunkTag.createMap(ImmutableMap.of(pChunkTag.createString("old_noise"), pChunkTag.createBoolean(STATUS_IS_OR_AFTER_NOISE.contains(s)))));
                  ...
                  if (chunkprototicklistfix$poormanspalettedcontainer != null) {
                     ...
                     if (flag && bitset.cardinality() != bitset.size()) {
                        ...
                        pChunkTag = pChunkTag.set("below_zero_retrogen", pChunkTag.createMap(ImmutableMap.of(pChunkTag.createString("target_status"), dynamic2, pChunkTag.createString("missing_bedrock"), pChunkTag.createLongList(LongStream.of(bitset.toLongArray())))));
   ...
   private static Dynamic<?> updateCarvingMasks(Dynamic<?> pDynamic, int p_184889_, int p_184890_) {
      Dynamic<?> dynamic = pDynamic.get("CarvingMasks").orElseEmptyMap();
      dynamic = dynamic.updateMapValues((p_196587_) -> {
         ...
         return Pair.of(p_196587_.getFirst(), pDynamic.createLongList(LongStream.of(along1)));
      });
      return pDynamic.set("CarvingMasks", dynamic);
   }

   private static Dynamic<?> addPaddingEntries(Dynamic<?> pDynamic, String p_184902_) {
      List<Dynamic<?>> list = pDynamic.get(p_184902_).orElseEmptyList().asStream().collect(Collectors.toCollection(ArrayList::new));
      if (list.size() == 24) {
         return pDynamic;
      } else {
         Dynamic<?> dynamic = pDynamic.emptyList();
         ...
         return pDynamic.set(p_184902_, pDynamic.createList(list.stream()));
      }
   }

   private static Dynamic<?> updateHeightmaps(Dynamic<?> pDynamic) {
      return pDynamic.update("Heightmaps", (p_196612_) -> {
   ...
   private static Dynamic<?> makeBiomeContainer(Dynamic<?> pDynamic, Int2IntFunction p_184896_) {
      ...
      Dynamic<?> dynamic = pDynamic.createList(int2intmap.keySet().stream().map((p_196598_) -> {
         return pDynamic.createString(BIOMES_BY_ID.getOrDefault(p_196598_.intValue(), "minecraft:plains"));
      }));
      ...
      if (i2 == 0) {
         ...
         Dynamic<?> dynamic1 = pDynamic.createLongList(Arrays.stream(along));
   ...
   private static Dynamic<?> padPaletteEntries(Dynamic<?> pPalette, Dynamic<?> pBlockStates, List<Dynamic<?>> p_196595_) {
      ...
      if (j <= (long)l) {
         ...
         return pPalette.createList(p_196595_.stream());
      }
   }

   public static int ceillog2(int pValue) {
      return pValue == 0 ? 0 : (int)Math.ceil(Math.log((double)pValue) / Math.log(2.0D));
   }
