...
public class ProcessorMailbox<T> implements ProfilerMeasured, ProcessorHandle<T>, AutoCloseable, Runnable {
   ...
   public static ProcessorMailbox<Runnable> create(Executor pDispatcher, String pName) {
      return new ProcessorMailbox<>(new StrictQueue.QueueStrictQueue<>(new ConcurrentLinkedQueue<>()), pDispatcher, pName);
   }
   ...
   private int pollUntil(Int2BooleanFunction pContinuePolling) {
      int i;
      for(i = 0; pContinuePolling.get(i) && this.pollTask(); ++i) {
      }
