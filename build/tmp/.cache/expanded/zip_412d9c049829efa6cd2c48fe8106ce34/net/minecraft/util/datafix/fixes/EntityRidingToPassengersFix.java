...
public class EntityRidingToPassengersFix extends DataFix {
   ...
   private <OldEntityTree, NewEntityTree, Entity> TypeRewriteRule cap(Schema pInputSchema, Schema pOutputSchema, Type<OldEntityTree> pOldEntityTreeType, Type<NewEntityTree> pNewEntityTreeType, Type<Entity> pEntityType) {
      Type<Pair<String, Pair<Either<OldEntityTree, Unit>, Entity>>> type = DSL.named(References.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Riding", pOldEntityTreeType)), pEntityType));
      Type<Pair<String, Pair<Either<List<NewEntityTree>, Unit>, Entity>>> type1 = DSL.named(References.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Passengers", DSL.list(pNewEntityTreeType))), pEntityType));
            ...
            return (p_145320_) -> {
               ...
               while(true) {
                  Either<List<NewEntityTree>, Unit> either = DataFixUtils.orElse(optional.map((p_145326_) -> {
                     Typed<NewEntityTree> typed = pNewEntityTreeType.pointTyped(p_15653_).orElseThrow(() -> {
                  ...
                  pair = (new Typed<>(pOldEntityTreeType, p_15653_, optional1.get())).getOptional(opticfinder).orElseThrow(() -> {
