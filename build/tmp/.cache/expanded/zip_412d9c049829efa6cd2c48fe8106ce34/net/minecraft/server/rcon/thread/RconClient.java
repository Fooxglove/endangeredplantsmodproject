...
public class RconClient extends GenericThread {
    ...
    */
   private void send(int pId, int p_11592_, String pMessage) throws IOException {
   ...
   private void sendCmdResponse(int pId, String pMessage) throws IOException {
      byte[] whole = pMessage.getBytes(StandardCharsets.UTF_8);
      int i = whole.length;
      int start = 0;
      do {
         int j = 4096 <= i ? 4096 : i;
         this.send(pId, 0, new String(java.util.Arrays.copyOfRange(whole, start, j+start), StandardCharsets.UTF_8));
         i -= j;
         start += j;
      } while(0 != i);
