...
public class EffectCommands {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> pDispatcher, CommandBuildContext pContext) {
      ...
      }).then(Commands.argument("effect", ResourceArgument.resource(pContext, Registries.MOB_EFFECT)).executes((p_248126_) -> {
      ...
      })))).then(Commands.literal("give").then(Commands.argument("targets", EntityArgument.entities()).then(Commands.argument("effect", ResourceArgument.resource(pContext, Registries.MOB_EFFECT)).executes((p_248127_) -> {
   ...
   private static int giveEffect(CommandSourceStack pSource, Collection<? extends Entity> pTargets, Holder<MobEffect> pEffect, @Nullable Integer pSeconds, int pAmplifier, boolean pShowParticles) throws CommandSyntaxException {
      ...
      for(Entity entity : pTargets) {
         if (entity instanceof LivingEntity) {
            MobEffectInstance mobeffectinstance = new MobEffectInstance(mobeffect, j, pAmplifier, false, pShowParticles);
   ...
   private static int clearEffects(CommandSourceStack pSource, Collection<? extends Entity> pTargets) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         if (pTargets.size() == 1) {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.effect.clear.everything.success.single", pTargets.iterator().next().getDisplayName());
            }, true);
         } else {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.effect.clear.everything.success.multiple", pTargets.size());
            }, true);
         }
   ...
   private static int clearEffect(CommandSourceStack pSource, Collection<? extends Entity> pTargets, Holder<MobEffect> pEffect) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         if (pTargets.size() == 1) {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.effect.clear.specific.success.single", mobeffect.getDisplayName(), pTargets.iterator().next().getDisplayName());
            }, true);
         } else {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.effect.clear.specific.success.multiple", mobeffect.getDisplayName(), pTargets.size());
            }, true);
         }
