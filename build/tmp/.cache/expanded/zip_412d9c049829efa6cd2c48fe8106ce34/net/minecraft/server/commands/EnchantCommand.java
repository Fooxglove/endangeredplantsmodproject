   ...
   private static int enchant(CommandSourceStack pSource, Collection<? extends Entity> pTargets, Holder<Enchantment> pEnchantment, int pLevel) throws CommandSyntaxException {
      ...
      if (pLevel > enchantment.getMaxLevel()) {
         ...
         if (i == 0) {
            ...
            if (pTargets.size() == 1) {
               pSource.sendSuccess(() -> {
                  return Component.translatable("commands.enchant.success.single", enchantment.getFullname(pLevel), pTargets.iterator().next().getDisplayName());
               }, true);
            } else {
               pSource.sendSuccess(() -> {
                  return Component.translatable("commands.enchant.success.multiple", enchantment.getFullname(pLevel), pTargets.size());
               }, true);
            }
