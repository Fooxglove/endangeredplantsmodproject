...
public class DamageCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> pDispatcher, CommandBuildContext pContext) {
      pDispatcher.register(Commands.literal("damage").requires((p_270872_) -> {
      ...
      }).then(Commands.argument("damageType", ResourceArgument.resource(pContext, Registries.DAMAGE_TYPE)).executes((p_270840_) -> {
   ...
   private static int damage(CommandSourceStack pSource, Entity pTarget, float pAmount, DamageSource pDamageType) throws CommandSyntaxException {
      if (pTarget.hurt(pDamageType, pAmount)) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.damage.success", pAmount, pTarget.getDisplayName());
         }, true);
         return 1;
      } else {
