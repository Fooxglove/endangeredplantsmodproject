...
public class CamelAi {
   ...
   private static void initCoreActivity(Brain<Camel> pBrain) {
      pBrain.addActivity(Activity.CORE, 0, ImmutableList.of(new Swim(0.8F), new CamelAi.CamelPanic(4.0F), new LookAtTargetSink(45, 90), new MoveToTargetSink(), new CountDownCooldownTicks(MemoryModuleType.TEMPTATION_COOLDOWN_TICKS), new CountDownCooldownTicks(MemoryModuleType.GAZE_COOLDOWN_TICKS)));
   }

   private static void initIdleActivity(Brain<Camel> pBrain) {
      pBrain.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, new AnimalMakeLove(EntityType.CAMEL, 1.0F)), Pair.of(2, new FollowTemptation((p_250812_) -> {
         return 2.5F;
   ...
   public static class RandomSitting extends Behavior<Camel> {
      ...
      protected boolean checkExtraStartConditions(ServerLevel pLevel, Camel pOwner) {
         return !pOwner.isInWater() && pOwner.getPoseTime() >= (long)this.minimalPoseTicks && !pOwner.isLeashed() && pOwner.onGround() && !pOwner.hasControllingPassenger();
      }
