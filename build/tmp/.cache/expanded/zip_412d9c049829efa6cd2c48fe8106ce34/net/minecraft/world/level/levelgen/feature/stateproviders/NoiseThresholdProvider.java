...
public class NoiseThresholdProvider extends NoiseBasedStateProvider {
   ...
   public BlockState getState(RandomSource pRandom, BlockPos pPos) {
      double d0 = this.getNoiseValue(pPos, (double)this.scale);
      if (d0 < (double)this.threshold) {
         return Util.getRandom(this.lowStates, pRandom);
      } else {
         return pRandom.nextFloat() < this.highChance ? Util.getRandom(this.highStates, pRandom) : this.defaultState;
      }
   }
}