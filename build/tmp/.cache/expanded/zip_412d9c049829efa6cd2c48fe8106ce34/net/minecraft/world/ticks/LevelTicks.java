...
public class LevelTicks<T> implements LevelTickAccess<T> {
   ...
   public LevelTicks(LongPredicate pTickCheck, Supplier<ProfilerFiller> pProfiler) {
   ...
   public void schedule(ScheduledTick<T> pTick) {
      long i = ChunkPos.asLong(pTick.pos());
      LevelChunkTicks<T> levelchunkticks = this.allContainers.get(i);
      if (levelchunkticks == null) {
         Util.pauseInIde(new IllegalStateException("Trying to schedule tick in not loaded position " + pTick.pos()));
      } else {
         levelchunkticks.schedule(pTick);
      }
   }
   ...
   private void drainFromCurrentContainer(Queue<LevelChunkTicks<T>> pContainersToTick, LevelChunkTicks<T> pLevelChunkTicks, long pGameTime, int pMaxAllowedTicks) {
      if (this.canScheduleMoreTicks(pMaxAllowedTicks)) {
         ...
         while(this.canScheduleMoreTicks(pMaxAllowedTicks)) {
            ...
            if (scheduledtick1 == null || scheduledtick1.triggerTick() > pGameTime || scheduledtick != null && ScheduledTick.INTRA_TICK_DRAIN_ORDER.compare(scheduledtick1, scheduledtick) > 0) {
               break;
            }
   ...
   public boolean hasScheduledTick(BlockPos pPos, T pType) {
      LevelChunkTicks<T> levelchunkticks = this.allContainers.get(ChunkPos.asLong(pPos));
      return levelchunkticks != null && levelchunkticks.hasScheduledTick(pPos, pType);
   }

   public boolean willTickThisTick(BlockPos pPos, T pType) {
      this.calculateTickSetIfNeeded();
      return this.toRunThisTickSet.contains(ScheduledTick.probe(pType, pPos));
   }
   ...
   public void clearArea(BoundingBox pArea) {
      Predicate<ScheduledTick<T>> predicate = (p_193241_) -> {
         return pArea.isInside(p_193241_.pos());
      };
   ...
   public void copyArea(BoundingBox pArea, Vec3i p_193244_) {
      this.copyAreaFrom(this, pArea, p_193244_);
   }

   public void copyAreaFrom(LevelTicks<T> pLevelTicks, BoundingBox pArea, Vec3i p_265318_) {
      List<ScheduledTick<T>> list = new ArrayList<>();
      Predicate<ScheduledTick<T>> predicate = (p_200922_) -> {
         return pArea.isInside(p_200922_.pos());
      };
      ...
      pLevelTicks.forContainersInArea(pArea, (p_200931_, p_200932_) -> {
