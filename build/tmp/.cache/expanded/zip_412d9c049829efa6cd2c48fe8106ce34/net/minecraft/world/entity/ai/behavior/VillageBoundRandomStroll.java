...
public class VillageBoundRandomStroll {
   ...
   public static OneShot<PathfinderMob> create(float pSpeedModifier, int pMaxHorizontalDist, int pMaxVericalDist) {
         ...
         return p_258869_.group(p_258869_.absent(MemoryModuleType.WALK_TARGET)).apply(p_258869_, (p_258863_) -> {
            return (p_258874_, p_258875_, p_258876_) -> {
               ...
               if (p_258874_.isVillage(blockpos)) {
                  vec3 = LandRandomPos.getPos(p_258875_, pMaxHorizontalDist, pMaxVericalDist);
               } else {
                  SectionPos sectionpos = SectionPos.of(blockpos);
                  SectionPos sectionpos1 = BehaviorUtils.findSectionClosestToVillage(p_258874_, sectionpos, 2);
                  if (sectionpos1 != sectionpos) {
                     vec3 = DefaultRandomPos.getPosTowards(p_258875_, pMaxHorizontalDist, pMaxVericalDist, Vec3.atBottomCenterOf(sectionpos1.center()), (double)((float)Math.PI / 2F));
                  } else {
                     vec3 = LandRandomPos.getPos(p_258875_, pMaxHorizontalDist, pMaxVericalDist);
                  }
               }
               ...
               p_258863_.setOrErase(Optional.ofNullable(vec3).map((p_258865_) -> {
                  return new WalkTarget(p_258865_, pSpeedModifier, 0);
               }));
               return true;
            };
         });
