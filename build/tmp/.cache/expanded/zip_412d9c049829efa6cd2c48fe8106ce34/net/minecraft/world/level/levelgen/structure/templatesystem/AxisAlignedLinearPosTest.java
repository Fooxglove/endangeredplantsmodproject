...
public class AxisAlignedLinearPosTest extends PosRuleTest {
   ...
   public boolean test(BlockPos pLocalPos, BlockPos pRelativePos, BlockPos pStructurePos, RandomSource pRandom) {
      Direction direction = Direction.get(Direction.AxisDirection.POSITIVE, this.axis);
      float f = (float)Math.abs((pRelativePos.getX() - pStructurePos.getX()) * direction.getStepX());
      float f1 = (float)Math.abs((pRelativePos.getY() - pStructurePos.getY()) * direction.getStepY());
      float f2 = (float)Math.abs((pRelativePos.getZ() - pStructurePos.getZ()) * direction.getStepZ());
      int i = (int)(f + f1 + f2);
