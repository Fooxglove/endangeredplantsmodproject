...
public class DualNoiseProvider extends NoiseProvider {
   ...
   public BlockState getState(RandomSource pRandom, BlockPos pPos) {
      ...
      for(int j = 0; j < i; ++j) {
         list.add(this.getRandomState(this.states, this.getSlowNoiseValue(pPos.offset(j * '\ud511', 0, j * '\u85ba'))));
      }
   ...
   protected double getSlowNoiseValue(BlockPos pPos) {
      return this.slowNoise.getValue((double)((float)pPos.getX() * this.slowScale), (double)((float)pPos.getY() * this.slowScale), (double)((float)pPos.getZ() * this.slowScale));
   }
}