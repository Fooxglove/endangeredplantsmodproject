...
public class EndPlacements {
   ...
   public static void bootstrap(BootstapContext<PlacedFeature> pContext) {
      ...
      PlacementUtils.register(pContext, END_GATEWAY_RETURN, holder1, RarityFilter.onAverageOnceEvery(700), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, RandomOffsetPlacement.vertical(UniformInt.of(3, 9)), BiomeFilter.biome());
      PlacementUtils.register(pContext, CHORUS_PLANT, holder2, CountPlacement.of(UniformInt.of(0, 4)), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, BiomeFilter.biome());
      PlacementUtils.register(pContext, END_ISLAND_DECORATED, holder3, RarityFilter.onAverageOnceEvery(14), PlacementUtils.countExtra(1, 0.25F, 1), InSquarePlacement.spread(), HeightRangePlacement.uniform(VerticalAnchor.absolute(55), VerticalAnchor.absolute(70)), BiomeFilter.biome());
   }
}