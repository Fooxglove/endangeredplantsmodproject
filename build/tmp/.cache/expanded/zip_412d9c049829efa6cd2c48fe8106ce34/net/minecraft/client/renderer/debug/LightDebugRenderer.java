...
public class LightDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, double pCamX, double pCamY, double pCamZ) {
      ...
      for(BlockPos blockpos1 : BlockPos.betweenClosed(blockpos.offset(-10, -10, -10), blockpos.offset(10, 10, 10))) {
         ...
         if (longset.add(k)) {
            DebugRenderer.renderFloatingText(pPoseStack, pBuffer, level.getChunkSource().getLightEngine().getDebugData(LightLayer.SKY, SectionPos.of(k)), (double)SectionPos.sectionToBlockCoord(SectionPos.x(k), 8), (double)SectionPos.sectionToBlockCoord(SectionPos.y(k), 8), (double)SectionPos.sectionToBlockCoord(SectionPos.z(k), 8), 16711680, 0.3F);
         }

         if (i != 15) {
            DebugRenderer.renderFloatingText(pPoseStack, pBuffer, String.valueOf(i), (double)blockpos1.getX() + 0.5D, (double)blockpos1.getY() + 0.25D, (double)blockpos1.getZ() + 0.5D, j);
         }
      }
