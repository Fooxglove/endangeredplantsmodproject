...
public class FrogModel<T extends Frog> extends HierarchicalModel<T> {
   ...
   public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      ...
      this.animate(pEntity.jumpAnimationState, FrogAnimation.FROG_JUMP, pAgeInTicks);
      this.animate(pEntity.croakAnimationState, FrogAnimation.FROG_CROAK, pAgeInTicks);
      this.animate(pEntity.tongueAnimationState, FrogAnimation.FROG_TONGUE, pAgeInTicks);
      if (pEntity.isInWaterOrBubble()) {
         this.animateWalk(FrogAnimation.FROG_SWIM, pLimbSwing, pLimbSwingAmount, 1.0F, 2.5F);
      } else {
         this.animateWalk(FrogAnimation.FROG_WALK, pLimbSwing, pLimbSwingAmount, 1.5F, 2.5F);
      }

      this.animate(pEntity.swimIdleAnimationState, FrogAnimation.FROG_IDLE_WATER, pAgeInTicks);
      this.croakingBody.visible = pEntity.croakAnimationState.isStarted();
   }
