...
public class GameLoadTimesEvent {
   ...
   public synchronized void beginStep(TelemetryProperty<GameLoadTimesEvent.Measurement> pMeasurement) {
      this.beginStep(pMeasurement, (p_286494_) -> {
   ...
   public synchronized void beginStep(TelemetryProperty<GameLoadTimesEvent.Measurement> pMeasurement, Stopwatch pStopwatch) {
      this.beginStep(pMeasurement, (p_286421_) -> {
         return pStopwatch;
      });
   }

   private synchronized void beginStep(TelemetryProperty<GameLoadTimesEvent.Measurement> pMeasurement, Function<TelemetryProperty<GameLoadTimesEvent.Measurement>, Stopwatch> pStopwatchGetter) {
   ...
   public synchronized void endStep(TelemetryProperty<GameLoadTimesEvent.Measurement> pMeasurement) {
      ...
      if (stopwatch == null) {
         LOGGER.warn("Attempted to end step for {} before starting it", (Object)pMeasurement.id());
      } else {
   ...
   public void send(TelemetryEventSender pSender) {
      pSender.send(TelemetryEventType.GAME_LOAD_TIMES, (p_286285_) -> {
         synchronized(this) {
