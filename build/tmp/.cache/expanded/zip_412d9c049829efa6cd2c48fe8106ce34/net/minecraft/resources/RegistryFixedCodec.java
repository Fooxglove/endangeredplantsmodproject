...
public final class RegistryFixedCodec<E> implements Codec<Holder<E>> {
         ...
         if (optional.isPresent()) {
            if (!pHolder.canSerializeIn(optional.get())) {
               return DataResult.error(() -> {
                  return "Element " + pHolder + " is not valid in current registry set";
               });
            }

            return pHolder.unwrap().map((p_206727_) -> {
               return ResourceLocation.CODEC.encode(p_206727_.location(), pOps, pValue);
            }, (p_274804_) -> {
               return DataResult.error(() -> {
   ...
   public <T> DataResult<Pair<Holder<E>, T>> decode(DynamicOps<T> pOps, T pValue) {
      if (pOps instanceof RegistryOps<?> registryops) {
         ...
         if (optional.isPresent()) {
            return ResourceLocation.CODEC.decode(pOps, pValue).flatMap((p_255515_) -> {
