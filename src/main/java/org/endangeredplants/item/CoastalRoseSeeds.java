...
public class CoastalRoseSeeds {
    public static final DeferredRegister<Item> ITEMS = DeferredRegister.create(ForgeRegistries.ITEMS, Endangeredplants.MODID);

    // 自定义种子物品类（保持相同逻辑）
    public static class RoseSeedsItem extends Item {
        public RoseSeedsItem(Properties properties) {
            super(properties);
        }
    ...
    public static final RegistryObject<Item> COASTAL_ROSE_SEEDS = ITEMS.register(
            "coastalroseseeds",
            () -> new RoseSeedsItem(new Item.Properties())
    );
