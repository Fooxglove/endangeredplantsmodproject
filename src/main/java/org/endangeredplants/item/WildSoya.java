...
import org.endangeredplants.block.WildSoyaBlock;

public class WildSoya {
    public static final DeferredRegister<Item> ITEMS = DeferredRegister.create(ForgeRegistries.ITEMS, Endangeredplants.MODID);

    // 自定义WildSoya物品类
    public static class WildSoyaItem extends Item {
        public WildSoyaItem(Properties properties) {
            super(properties);
        }
        ...
        public InteractionResult useOn(UseOnContext context) {
            ...
            if ((level.getBlockState(pos).is(Blocks.DIRT) ||
                ...
                // 在草方块或泥土上方生成WildSoyaBlock
                level.setBlock(abovePos, WildSoyaBlock.WILD_SOYA_BLOCK.get().defaultBlockState(), 1);
            ...
            else if (level.getBlockState(pos).is(Blocks.FARMLAND) &&
                    level.getBlockState(abovePos).isAir()) {
    ...
    // 注册物品
    public static final RegistryObject<Item> WILD_SOYA = ITEMS.register(
            "wildsoya",
            () -> new WildSoyaItem(new Item.Properties())
    );
